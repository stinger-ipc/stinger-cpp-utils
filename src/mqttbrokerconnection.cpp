    ...
    : _mosq(NULL), _host(host), _port(port), _clientId(clientId), _logLevel(LOG_NOTICE)
{
    ...
    mosquitto_connect_v5_callback_set(_mosq, [](struct mosquitto *mosq, void *user, int rc, int flags, const mosquitto_property *props)
    {
        ...
        while(!thisClient->_msgQueue.empty())
        {
            ...
            {
                mosquitto_property_add_string(&propList, MQTT_PROP_RESPONSE_TOPIC, msg.properties.responseTopic->c_str());
            }
            if (msg.properties.messageExpiryInterval)
            {
                mosquitto_property_add_int32(&propList, MQTT_PROP_MESSAGE_EXPIRY_INTERVAL, *msg.properties.messageExpiryInterval);
            }
            if (msg.properties.debugInfo)
            {
                mosquitto_property_add_string_pair(&propList, MQTT_PROP_USER_PROPERTY, "DebugInfo", msg.properties.debugInfo->c_str());
            }
            ...
            if (msg.properties.propertyVersion)
            {
                ...
                mosquitto_property_add_string_pair(&propList, MQTT_PROP_USER_PROPERTY, "PropertyVersion", propertyVersionStr.c_str());
            }
            if (msg.properties.version)
            {
                mosquitto_property_add_string_pair(&propList, MQTT_PROP_USER_PROPERTY, "Version", msg.properties.version->c_str());
            }
            int mid;
            mosquitto_publish_v5(mosq, &mid, msg.topic.c_str(), msg.payload.size(), msg.payload.c_str(), msg.qos, msg.retain, propList);
            mosquitto_property_free_all(&propList);
    ...
    mosquitto_message_v5_callback_set(_mosq, [](struct mosquitto *mosq, void *user, const struct mosquitto_message *mmsg, const mosquitto_property *props)
    {
        ...
        for (prop = props; prop != NULL; prop = mosquitto_property_next(prop))
        {
            ...
            else if (mosquitto_property_identifier(prop) == MQTT_PROP_MESSAGE_EXPIRY_INTERVAL)
            {
                ...
                if (mosquitto_property_read_int32(prop, MQTT_PROP_MESSAGE_EXPIRY_INTERVAL, &messageExpiryInterval, false))
                {
                    mqttProps.messageExpiryInterval = messageExpiryInterval;
                }
            }
        }
        auto msg = MqttMessage(topic, payload, mmsg->qos, mmsg->retain, mqttProps);
        ...
        const MqttMessage& message)
{
    ...
    {
        mosquitto_property_add_int32(&propList, MQTT_PROP_MESSAGE_EXPIRY_INTERVAL, *message.properties.messageExpiryInterval);
    }
    if (message.properties.debugInfo)
    {
        mosquitto_property_add_string_pair(&propList, MQTT_PROP_USER_PROPERTY, "DebugInfo", message.properties.debugInfo->c_str());
    }
    ...
    {
        mosquitto_property_add_string_pair(&propList, MQTT_PROP_USER_PROPERTY, "Version", message.properties.version->c_str());
    }
    int rc = mosquitto_publish_v5(_mosq, &mid, message.topic.c_str(), message.payload.size(), message.payload.c_str(), message.qos, message.retain, propList);
    if (propList) {
        mosquitto_property_free_all(&propList);
    }
